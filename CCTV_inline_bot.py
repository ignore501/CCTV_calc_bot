...
def create_options_keyboard():
	...
	for opt_type, opt_data in options.items():
		keyboard.append(InlineKeyboardButton(opt_data["label"], callback_data=opt_type))
	keyboard.append(InlineKeyboardButton("Рассчитать", callback_data="Рассчитать"))
	keyboard = [
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
	...
	await update.effective_chat.send_message(
		...
		f"Для начала расчёта стоимости системы видеонаблюдения выберите тип помещения:</b>",
...
async def restart(update: Update, context: ContextTypes.DEFAULT_TYPE):
	try:
		for i in range(update.callback_query.message.message_id, context.user_data['first_mes_id'], -1):
